(() => {
  ...
  function pageTransitionOut() {
    ...
    const textAnimationTransition = document.querySelectorAll("[data-split-animation][data-lines-split]");
  ...
  function initPageTransitions() {
    async function commonLeaveBefore(data) {
      ...
      document.querySelectorAll("[data-navigation-status]").forEach((el) => el.setAttribute("data-navigation-status", "is-closed"));
  ...
  function initScrollTriggerAnimations() {
    ...
    if (translateElements.length && scrollContainer) {
      ...
      const inverseTranslate = scrollContainer.dataset.inverseTranslate ? parseFloat(scrollContainer.dataset.inverseTranslate) : -targetTranslate;
      ...
      isSmallScreen.add("(min-width: 768px)", () => {
        if (!prefersReducedMotion) {
          translateElements.forEach((element) => {
            gsap.fromTo(element, { y: inverseTranslate }, {
              y: targetTranslate,
              ease: "none",
              scrollTrigger: {
  ...
  function initWorksPage() {
    ...
    function getScrollAmount() {
      const visibleItems = Array.from(items).filter((item) => item.getAttribute("data-filter-status") === "active");
      if (visibleItems.length === 0)
        return 0;
      const wrapperWidth = wrapper.scrollWidth;
      const viewportWidth = window.innerWidth;
      const firstItemLeft = visibleItems[0].getBoundingClientRect().left;
      const lastItemMargin = parseFloat(getComputedStyle(visibleItems[visibleItems.length - 1]).marginRight);
    ...
    function showOverscrollContent() {
      ...
      const overscrollContent = document.querySelector(".works_overscroll_content");
    ...
    function initOverscrollNavigation() {
      ...
      function startDecay() {
        ...
        if (progress < 0.5) {
          ...
          const translateProgress = -100 + progress / CONFIG.progressThreshold * 100;
      ...
      function updateProgress(delta) {
        ...
        if (velocity > CONFIG.minVelocity) {
          const velocityBoost = Math.min(velocity * CONFIG.velocityMultiplier, 4);
          const progressIncrease = delta * velocityBoost / CONFIG.sensitivity;
          progress = Math.min(progress + progressIncrease, CONFIG.progressThreshold);
          clearTimeout(decayTimeout);
          const translateProgress = -100 + progress / CONFIG.progressThreshold * 100;
          updateProgressBar(translateProgress);
          scheduleDecay();
